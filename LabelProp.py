...
import monai
import kornia
from voxelmorph2d import VxmDense,SpatialTransformer,NCC,Grad,Dice
...
class LabelProp(pl.LightningModule):
    ...
    def __init__(self,n_channels=1,n_classes=2,learning_rate=5e-3,weight_decay=1e-8,way='up',shape=256,selected_slices=None,losses={}):
        super().__init__()
        ...
        self.registrator= VxmDense(shape,bidir=self.bidir,int_downsize=1,int_steps=7)
        self.nearest_transformer=SpatialTransformer(shape,mode='bilinear') #garbage
    ...
    def training_step(self, batch, batch_nb):
        ...
        for chunk in chunks:
            ...
            if not self.way=='down':
                ...
                pos_y=Y[:,:,chunk[0],...]
                for pos in pos_seq:
                    pos_x=self.apply_trans(pos_x,pos)#torch.cat((pos_x,self.apply_trans(pos_x[:,:,-1,...],pos).unsqueeze(2)),2)
                    pos_y=self.apply_trans(pos_y,pos)#torch.cat((pos_y,(self.apply_trans(pos_y[:,:,-1,...],pos).unsqueeze(2))),2)
                
                if self.losses['compo-reg-up']:
                    loss+=self.compute_loss(pos_x,X[:,:,chunk[1],...])
                if self.losses['compo-dice-up']:
                    loss+=self.compute_loss(y=pos_y,y2=Y[:,:,chunk[1],...])

            if not self.way=='up':
                neg_x=X[:,:,chunk[1],...]
                neg_y=Y[:,:,chunk[1],...]
                for neg in reversed(neg_seq):
                    neg_x=self.apply_trans(neg_x,neg)#torch.cat((self.apply_trans(neg_x[:,:,0,...],neg).unsqueeze(2),neg_x),2)
                    neg_y=self.apply_trans(neg_y,neg)#torch.cat(((self.apply_trans(neg_y[:,:,0,...],neg).unsqueeze(2)),neg_y),2)
                if self.losses['compo-reg-down']:
                    loss+=self.compute_loss(neg_x,X[:,:,chunk[0],...])
                if self.losses['compo-dice-down']:
                    loss+=self.compute_loss(y=neg_y,y2=Y[:,:,chunk[0],...])
